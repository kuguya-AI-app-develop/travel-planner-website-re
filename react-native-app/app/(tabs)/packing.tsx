...
import { Plan, PackingCategory } from '../../types';
...
export default function PackingScreen() {
  ...
  const packing = selectedPlan?.packing ?? [];

  const handleUpdateCategories = useCallback(
    async (categories: PackingCategory[]) => {
      if (!selectedPlan) return;
      try {
        await updatePlan(selectedPlan.id, { data: { packing: categories } });
      } catch (e: unknown) {
  ...
  return (
    <View style={styles.container}>
      ...
      {!selectedPlan ? (
        <View style={styles.center}>
          <Text style={styles.hint}>请选择一个计划查看打包清单</Text>
        </View>
      ) : (
        <PackingView categories={packing} onUpdateCategories={handleUpdateCategories} />
      )}
    </View>
  );
}
