...
import { Plan, Document } from '../../types';
...
export default function DocumentsScreen() {
  ...
  const handlePressDoc = useCallback((doc: Document) => {
    setEditingDoc(doc);
    setShowModal(true);
  }, []);
  ...
  const handleSave = useCallback(
    async (data: Omit<Document, 'id'> & { id?: number }) => {
      ...
      if (data.id) {
        ...
        updated = [...documents, { ...data, id: newId }];
      }
  ...
  return (
    <View style={styles.container}>
      ...
      {!selectedPlan ? (
        <View style={styles.center}>
          <Text style={styles.hint}>请选择一个计划查看文档</Text>
        </View>
      ) : (
        <>
          <DocumentsView documents={documents} onPressDocument={handlePressDoc} />
